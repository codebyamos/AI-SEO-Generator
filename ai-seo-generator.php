...
// Load Composer dependencies from centralized vendor location
// First try central vendor (outside plugin), then fall back to plugin's vendor
$vendor_paths = array(
    ABSPATH . '../vendor/autoload.php',           // /home/.../public_html/vendor/
    ABSPATH . 'vendor/autoload.php',              // /home/.../public_html/wp-content/../vendor/
    AI_SEO_PLUGIN_DIR . 'vendor/autoload.php'     // /home/.../plugins/ai-seo-generator/vendor/
);
            ...
            var injectButtons = function() {
                ...
                if (!$('#ai-image-widget').length) {
                    var unifiedWidget = '<div id="ai-image-widget" style="position: fixed; bottom: 20px; right: 20px; z-index: 99999; font-family: -apple-system, BlinkMacSystemFont, sans-serif;">' +
                        '<div id="ai-widget-toggle" style="width: 56px; height: 56px; background: #000000; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(0,0,0,0.4); transition: transform 0.2s;" title="AI Image Generator">' +
                        ...
                        '<button type="button" id="ai-generate-image" data-post-id="<?php echo esc_attr($post->ID); ?>" style="flex: 1; padding: 10px 15px; background: #000000; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 14px;">Generate</button></div>' +
                        ...
                        '<button type="button" id="ai-use-featured" style="width: 100%; padding: 10px; background: #000000; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600;">⭐ Use as Featured Image</button>' +
